...
function App(el = "canvas-grid") {
  ...
  function init() {
    ...
    renderer.domElement.addEventListener("mousemove", (e) => {
      ...
      ripple.addDrop(gp.x, gp.y, 0.087, 0.1123);
    });
  ...
  function initScene() {
    ...
    let pointLight1 = new THREE.PointLight(0x88bfb8);
    pointLight1.position.set(-wWidth / 2, wHeight / 2, 50);
    scene.add(pointLight1);

    let pointLight2 = new THREE.PointLight(0x5da39d);
    pointLight2.position.set(wWidth / 2, wHeight / 2, 50);
    scene.add(pointLight2);

    let pointLight3 = new THREE.PointLight(0x3d8e86);
    pointLight3.position.set(-wWidth / 2, -wHeight / 2, 50);
    scene.add(pointLight3);

    let pointLight4 = new THREE.PointLight(0x206864);
    pointLight4.position.set(wWidth / 2, -wHeight / 2, 50);
    scene.add(pointLight4);

    // renderer.domElement.addEventListener("mouseup", (e) => {
    //   pointLight1.color = new THREE.Color(
    //     `#${parseInt(Math.random() * 0xffffff).toString(16)}`
    //   );
    //   pointLight2.color = new THREE.Color(
    //     `#${parseInt(Math.random() * 0xffffff).toString(16)}`
    //   );
    //   pointLight3.color = new THREE.Color(
    //     `#${parseInt(Math.random() * 0xffffff).toString(16)}`
    //   );
    //   pointLight4.color = new THREE.Color(
    //     `#${parseInt(Math.random() * 0xffffff).toString(16)}`
    //   );
    // });
    ...
    nx = Math.round(gridWidth / 40);
    ny = Math.round(gridHeight / 5);
    dx = gridWWidth / nx;
    dy = gridWHeight / ny;
    for (let i = 0; i <= nx; i++) {
      const geometry = new THREE.BufferGeometry();
